  ...
  # Combing the match file to the data file (from cov_fetch)
  level_annual_wide_matched <-  hy_annual_wide %>% 
                                select(-cov_water_flow_yearly_mean) %>% 
                                filter(Year %notin% c(2021, 2022)) %>% 
                                full_join(get_water_office_level_2021_2022) %>% 
                                right_join(loc_matching_level_simple) %>% 
                                select(-STATION_NUMBER)
  
  flow_annual_wide_matched <-  hy_annual_wide %>% 
                               select(-cov_water_level_yearly_mean) %>% 
                               filter(Year %notin% c(2021, 2022)) %>% 
                               full_join(get_water_office_flow_2021_2022) %>% 
                               right_join(loc_matching_level_simple) %>% 
                               select(-STATION_NUMBER)
