...
# stock_list
stock_list<-fcs_covariates_long %>% select(Stock_ERA) %>% unique()
fcs_covariates_combined_BQR<-fcs_covariates_combined %>% filter(Stock_ERA == "BQR")

fcs_covariates_combined_list<-list()

## making some functions:


for(stock in stock_list) {
  fcs_covariates_combined_list[stock]<-fcs_covariates_combined %>% filter(Stock_ERA == stock)
}
...
matches_MGS_4_run_year = unique(MGS_4_sample_age_covariates_run_year$Covariate)
matches_MGS_4_run_lead1_year = unique(MGS_4_sample_age_covariates_run_lead1_year$Covariate)
matches_MGS_4_run_lead2_year = unique(MGS_4_sample_age_covariates_run_lead2_year$Covariate)
matches_MGS_4_brood_year =unique(MGS_4_sample_age_covariates_brood_year$Covariate)
matches_MGS_4_brood_lag1_year = unique(MGS_4_sample_age_covariates_brood_lag1_year$Covariate)
matches_MGS_4_brood_lag2_year = unique(MGS_4_sample_age_covariates_brood_lag2_year$Covariate)
...
#Run Year
for(Covariate_ in matches_MGS_4_run_year) {
...
#Run Year lead 1
for(Covariate_ in matches_MGS_4_run_lead1_year) {
...
#Run Year lead 2
for(Covariate_ in matches_MGS_4_run_lead2_year) {
...
#Brood Year
for(Covariate_ in matches_MGS_4_brood_year) {
...
#Brood Year lag 1
for(Covariate_ in matches_MGS_4_brood_lag1_year) {
...
#Brood Year lag 2
for(Covariate_ in matches_MGS_4_brood_lag2_year) {
